 ...
 * Version: 1.2 - Con logs mejorados y errores corregidos
 */
...
class LicenseAPI {
    ...
    public function handleRequest() {
        ...
        // Log de la petición con información mejorada
    ...
    private function validateLicense() {
        ...
        if (!$license) {
            $this->logActivity(null, null, 'validation', 'failure', 'License key not found', $domain);
            $this->sendError('Invalid license key', 404);
        }
        
        // Verificar estado de la licencia
        if ($license['status'] !== 'active') {
            $this->logActivity($license['id'], null, 'validation', 'failure', 'License not active: ' . $license['status'], $domain);
        ...
        if ($license['current_period_status'] === 'pending') {
            $this->logActivity($license['id'], null, 'validation', 'failure', 'License period has not started yet', $domain);
        ...
        if ($license['period_status'] === 'expired') {
            $this->logActivity($license['id'], null, 'validation', 'failure', 'License expired', $domain);
        ...
        $this->logActivity($license['id'], null, 'validation', 'success', 'License validated for domain: ' . $domain, $domain);
    ...
    private function activateLicense() {
        ...
        if (!$license) {
            $this->logActivity(null, null, 'activation', 'failure', 'Invalid license key for activation', $domain);
            $this->sendError('Invalid or inactive license key', 404);
        }
        
        // Verificar si la licencia está en su período válido
        if ($license['current_period_status'] === 'pending') {
            $this->logActivity($license['id'], null, 'activation', 'failure', 'License period has not started yet', $domain);
            $this->sendError('License period has not started yet. Valid from: ' . date('Y-m-d H:i', strtotime($license['start_date'])), 403);
        }
        
        // Verificar expiración
        if ($license['period_status'] === 'expired') {
            $this->logActivity($license['id'], null, 'activation', 'failure', 'Attempted activation of expired license', $domain);
            $this->sendError('License has expired on: ' . date('Y-m-d H:i', strtotime($license['expires_at'])), 403);
        }
        ...
        if ($existing) {
            ...
            $this->logActivity($license['id'], $existing['id'], 'activation', 'success', 'License reactivated for domain: ' . $domain, $domain);
        ...
        if ($activations['count'] >= $license['max_domains']) {
            $this->logActivity($license['id'], null, 'activation', 'failure', 'Domain limit exceeded for activation', $domain);
            $this->sendError('Maximum domains limit reached', 403);
        }
        ...
        if ($stmt->execute()) {
            ...
            $this->logActivity($license['id'], $activation_id, 'activation', 'success', 'License activated for domain: ' . $domain, $domain);
            ...
            $this->logActivity($license['id'], null, 'activation', 'failure', 'Database error during activation', $domain);
    ...
    private function verifyLicense() {
        $license_key = $this->getParam('license_key');
        $domain = $this->getParam('domain');
        $software = $this->getParam('software', 'Unknown');
        $version = $this->getParam('version', '1.0');
        ...
        if (!$result) {
            $this->logActivity(null, null, 'verification', 'failure', 'License not found for verification', $domain);
        ...
        // Verificar estado de la licencia
        if ($license['status'] !== 'active') {
            $this->logActivity($license['id'], $license['activation_id'], 'verification', 'failure', 'License not active', $domain);
            $this->sendError('License is not active', 403);
        }
        
        // Verificar activación
        if (!$license['activation_id'] || $license['activation_status'] !== 'active') {
            $this->logActivity($license['id'], null, 'verification', 'failure', 'No active activation for domain', $domain);
            $this->sendError('License not activated for this domain', 403);
        }
        
        // Verificar si la licencia está en su período válido
        if ($license['current_period_status'] === 'pending') {
            $this->logActivity($license['id'], $license['activation_id'], 'verification', 'failure', 'License period has not started yet', $domain);
            $this->sendError('License period has not started yet', 403);
        }
        
        // Verificar expiración
        if ($license['period_status'] === 'expired') {
            $this->logActivity($license['id'], $license['activation_id'], 'verification', 'failure', 'License expired', $domain);
            $this->sendError('License has expired', 403);
        }
        
        // Actualizar última verificación con información detallada
        ...
        // Log detallado de verificación exitosa
        $verification_details = sprintf(
            'License verified successfully - Software: %s v%s, Client: %s',
            $software,
            $version,
            $license['client_name']
        );
    ...
    private function deactivateLicense() {
        ...
        if ($stmt->execute()) {
            $this->logActivity($result['license_id'], $result['id'], 'deactivation', 'success', 'License deactivated for domain: ' . $domain, $domain);
            $this->sendSuccess([
                'deactivated' => true,
    ...
    private function getStatus() {
        ...
        ");
        $verification_stats = $stmt->fetch_assoc();
        ...
        $this->sendSuccess([
            ...
            'verification_stats' => [
                'total_verifications' => (int)$verification_stats['total_verifications'],
                'verifications_24h' => (int)$verification_stats['verifications_24h'],
                'last_verification' => $verification_stats['last_verification']
            ]
        ]);
    }
