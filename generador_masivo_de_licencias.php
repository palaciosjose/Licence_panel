...
require_once 'whatsapp_config.php'; // Necesario si LicenseManager usa esta configuración
require_once 'LicenseManager.class.php'; 

// Inicializar el gestor de licencias para este script
$licenseManager = new LicenseManager($license_db_config, $whatsapp_config); 

// Verificar autenticación
if (!$licenseManager->isLoggedIn()) {
    // Si no está autenticado, redirigir al panel de administración para login
...
// === INICIO: IMPLEMENTACIÓN DEL PATRÓN POST/REDIRECT/GET ===
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['generate_bulk'])) {
    $bulk_config = [
        ...
        'duration_days' => $_POST['duration_days'] === 'custom' && !empty($_POST['custom_duration']) ? (int)$_POST['custom_duration'] : ((!empty($_POST['duration_days']) && $_POST['duration_days'] !== 'custom') ? (int)$_POST['duration_days'] : null),
    ...
    $generated_licenses_data = generateBulkLicenses($bulk_config); 
    
    $_SESSION['last_bulk_generation_results'] = $generated_licenses_data;
    
    header('Location: ' . $_SERVER['PHP_SELF'] . '?action=results');
    exit;
}

if ($current_action === 'results' && isset($_SESSION['last_bulk_generation_results'])) {
    $generated_licenses = $_SESSION['last_bulk_generation_results'];
    unset($_SESSION['last_bulk_generation_results']);
} else if ($current_action === 'results') {
    header('Location: ' . $_SERVER['PHP_SELF'] . '?action=form'); 
    exit;
}

// === FIN: IMPLEMENTACIÓN DEL PATRÓN POST/REDIRECT/GET ===


function generateBulkLicenses($config) {
    global $licenseManager; // Asegúrate de que licenseManager esté disponible en la función
...
if ($current_action === 'export' && isset($generated_licenses)) { 
    exportLicensesToCSV(
        $generated_licenses['licenses'],
        $generated_licenses['config']
    );
}
?>
...
<head>
    ...
    <style>
        ...
        /* Nuevo estilo para la tabla de resultados del generador masivo */
        .period-compact-table {
            font-size: 0.75rem; /* Letra más pequeña */
            line-height: 1.2; /* Menor espacio entre líneas */
        }
        .period-compact-table strong {
            display: inline-block; /* Para que la etiqueta strong no ocupe toda la línea */
            width: 40px; /* Ancho fijo para alinear los valores */
        }
    </style>
</head>
<body class="py-4">
    <div class="container">
        ...
        <?php elseif ($current_action === 'results'): ?>
            <div class="generator-card p-4 mx-auto" style="max-width: 1200px;">
                ...
                <?php 
                // NO NECESITAMOS GUARDAR EN SESIÓN AQUÍ, YA QUE SE RECUPERÓ ANTES DE LA REDIRECCIÓN
    ...
    <script>
        ...
        function updatePeriodPreview() {
            ...
            if (!startDate) {
                previewContent.innerHTML = '<small class="text-muted">Selecciona fecha y duración para ver la vista previa</small>';
                return;
            }
